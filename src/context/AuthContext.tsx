...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [mustOpenInTelegram, setMustOpenInTelegram] = useState(false);
  ...
  // Telegram WebApp Auto-login & Registration logic
  useEffect(() => {
    let isMounted = true;
    ...
    const initTelegramWebApp = async () => {
      ...
      if (tg) {
        ...
        if (tgUser) {
          ...
          setMustOpenInTelegram(false);

          const tgEmail = `tg_${tUser.telegramId}@telegram.davidstore.com`;
          const tgPassword = `telegram_${tUser.telegramId}_secure`;

          console.log(`[TELEGRAM MINI APP] Detected user: ${tUser.firstName} (${tUser.telegramId})`);

          // Only proceed if not already logged in as this Telegram user
          if (auth.currentUser && auth.currentUser.email === tgEmail) {
            console.log(`[TELEGRAM MINI APP] Already logged in as ${tgEmail}`);
            return;
          }

          if (auth.currentUser) {
            console.log(`[TELEGRAM MINI APP] Signing out currently active different user`);
            await signOut(auth);
          }

          setLoading(true);
          try {
            console.log(`[TELEGRAM MINI APP] Logging in ${tgEmail}`);
            await signInWithEmailAndPassword(auth, tgEmail, tgPassword);
            if (auth.currentUser) {
              await updateDoc(doc(db, "users", auth.currentUser.uid), { telegramId: tUser.telegramId });
            }
          } catch (signInErr: any) {
            if (signInErr.code === 'auth/user-not-found' || signInErr.code === 'auth/invalid-credential' || signInErr.code === 'auth/invalid-email' || signInErr.code === 'auth/cannot-find-user-entry') {
              console.log(`[TELEGRAM MINI APP] Creating virtual user: ${tgEmail}`);
              try {
                isRegisteringRef.current = true;
                const userCred = await createUserWithEmailAndPassword(auth, tgEmail, tgPassword);
                const firebaseUser = userCred.user;
                const profileRef = doc(db, 'users', firebaseUser.uid);
                const now = Date.now();
                const fullName = `${tUser.firstName} ${tUser.lastName}`.trim() || 'Utilisateur Telegram';
                ...
                const newProfile: UserProfile = {
                  ...
                  telegramId: tUser.telegramId,
                  username: tUser.username,
                  languageCode: tUser.languageCode,
                  phone: '',
                  telephone: 'Numéro non renseigné',
                  addresses: [],
                  wishlist: [],
                  createdAt: now,
                  dateCreation: now,
                ...
                console.log(`[TELEGRAM MINI APP] Profile successfully created for ${tgEmail}`);
              } catch (signUpErr) {
                console.error(`[TELEGRAM MINI APP] Signup failed:`, signUpErr);
              } finally {
                isRegisteringRef.current = false;
              }
            } else {
              console.error(`[TELEGRAM MINI APP] Login failed:`, signInErr);
            }
          } finally {
            if (isMounted) {
              setLoading(false);
          ...
          // No tgUser - check if dev to simulate
          const isDev = window.location.hostname.includes('ais-dev-') || window.location.hostname.includes('ais-pre-') || window.location.hostname.includes('localhost');
          if (!isDev) {
            if (isMounted) {
              setMustOpenInTelegram(true);
              setLoading(false);
            }
          }
        }
      } else {
        // No Telegram SDK - check if dev to simulate
        const isDev = window.location.hostname.includes('ais-dev-') || window.location.hostname.includes('ais-pre-') || window.location.hostname.includes('localhost');
        if (!isDev) {
          if (isMounted) {
            setMustOpenInTelegram(true);
  ...
  useEffect(() => {
    ...
    // Safety timeout: if auth/Firestore doesn't respond in 5s, stop loading to show the app
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (!isRegisteringRef.current) {
          let profileRef = doc(db, 'users', currentUser.uid);
          ...
          try {
            ...
            if (isMounted) {
              if (profileSnap && profileSnap.exists()) {
                ...
                setProfile(prev => {
                  ...
                  if (currentUser.email?.startsWith('tg_') && currentUser.email?.endsWith('@telegram.davidstore.com')) {
                    tgIdStr = currentUser.email.split('@')[0].replace('tg_', '');
                    isTgUser = true;
                    defaultName = 'Utilisateur Telegram';
                  } else if (currentUser.email?.endsWith('@davidstore.com')) {
                    const phone = currentUser.email.split('@')[0];
            ...
            if (error.code === 'unavailable' || error.message?.includes('offline')) {
              console.warn("Firestore is offline, could not fetch profile. Continuing with partial state.");
            } else {
  ...
  const updateProfile = React.useCallback(async (updatedData: Partial<UserProfile>, photo?: File, bypassEmailVerification?: boolean, forceOverwriteEmail?: boolean) => {
    ...
    if (user.email?.startsWith('tg_') && user.email?.endsWith('@telegram.davidstore.com')) {
      const tgIdStr = user.email.split('@')[0].replace('tg_', '');
      profileRef = doc(db, 'users', `tg_${tgIdStr}`);
    }
    try {
  ...
  const value = React.useMemo(() => ({ 
    ...
    telegramUser,
    mustOpenInTelegram
  }), [user, profile, loading, isAdmin, signInWithPhone, signUpWithPhone, logout, toggleWishlist, updateProfile, sendPasswordReset, maintenanceMode, maintenanceMessage, maintenanceChangelog, maintenanceLoaded, setMaintenance, mergeAccounts, isTelegramWebApp, telegramUser, mustOpenInTelegram]);
