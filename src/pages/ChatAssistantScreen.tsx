...
import { EX_PRODUCTS } from '../services/mockData';
...
export const ChatAssistantScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCatalog = async () => {
      try {
        ...
        setCatalogProducts(prods.length > 0 ? prods : EX_PRODUCTS);
      } catch (err) {
        console.error("Could not load product catalog in assistant chat screen:", err);
        setCatalogProducts(EX_PRODUCTS);
      }
    };
    fetchCatalog();
  }, []);
  ...
  const handleSendWithPrompt = async (messageText: string) => {
    ...
    try {
      ...
      const effectiveCatalog = catalogProducts.length > 0 ? catalogProducts : EX_PRODUCTS;
      const productsContext = effectiveCatalog.map(p => ({
        id: p.id,
        name: p.name,
        price: p.price,
        category: p.category,
